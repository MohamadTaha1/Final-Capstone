...
const DailySpecialsPage = () => {
    ...
    useEffect(() => {
        const fetchOwnerRestaurants = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/owner-restaurants', {
    ...
    useEffect(() => {
      ...
      const fetchDailySpecials = async () => {
        try {
            const response = await axios.get(`http://localhost:8000/api/restaurants/${restaurantId}/daily-specials`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (response.data.length > 0) {
  ...
  const handleSpecialChange = (day, dishId, price) => {
    ...
    const handleSubmit = async (e) => {
      ...
      const formattedData = Object.entries(specials).map(([day, { dishId, price }]) => ({
      ...
      try {
          await axios.post('http://localhost:8000/api/daily-specials', formattedData, {
              headers: { Authorization: `Bearer ${token}` },
          });
...
const handleConfirmClick = async (dayOfWeek) => {
  ...
  try {
      await axios.patch(`http://localhost:8000/api/daily-specials/${specialToUpdate.id}`, {
  ...
  if (scheduleExists) {
      return (
          ...
          <div className="container mx-auto p-4">
              <h2 className="text-2xl font-bold text-center mb-4">Your Restaurants Daily Specials</h2>
              <div className="overflow-x-auto">
                  <table className="min-w-full bg-white shadow-md rounded-lg">
                      ...
                      <tbody className="text-gray-600 text-sm font-light">
                        {Object.entries(specials).map(([dayOfWeek, { dishId, price,}]) => (
                            <tr key={dayOfWeek} className="border-b border-gray-200 hover:bg-gray-100">
                                <td className="py-3 px-6 text-left whitespace-nowrap">{dayOfWeek}</td>
                                <td className="py-3 px-6 text-left">
                                    {editing === dayOfWeek ? (
                                        <select
                                            ...
                                            onChange={(e) => handleSpecialChange(dayOfWeek, e.target.value, price)}
                                        >
                                            {dishes.map(dish => (
                                        ...
                                        dishes.find(dish => dish.id === dishId)?.name || 'Dish not found'
                                    )}
                                </td>
                                <td className="py-3 px-6 text-center">
                                    {editing === dayOfWeek ? (
                                        <input
                                            ...
                                            onChange={(e) => handleSpecialChange(dayOfWeek, dishId, e.target.value)}
                                        />
                                    ) : (
                                        `$${price}`
                                    )}
                                ...
                                <td className="py-3 px-6 text-center">
                                    {editing === dayOfWeek ? (
                                        <button onClick={() => handleConfirmClick(dayOfWeek)} className="bg-green-500 text-white active:bg-green-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1" type="button" style={{ transition: "all .15s ease" }}>
                                            Confirm
                                        </button>
                                    ) : (
                                        <button onClick={() => handleEditClick(dayOfWeek)} className="bg-blue-500 text-white active:bg-blue-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1" type="button" style={{ transition: "all .15s ease" }}>
                                            Edit
                                        </button>
                                    )}
                                </td>
  ...
  return (
    <div className="container mx-auto p-6 bg-white shadow-md rounded-lg">
        <h2 className="text-3xl font-semibold text-center text-gray-700 mb-6">Manage Daily Specials for Your Restaurant</h2>
            ...
            {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map((day, index) => (
                <div key={day} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                    ...
                    <select
                        ...
                        onChange={(e) => handleSpecialChange(index, e.target.value, specials[index]?.price || '')}
                    >
                    ...
                    <input
                        ...
                        onChange={(e) => handleSpecialChange(index, specials[index]?.dishId || '', e.target.value)}
                        placeholder="Price"
                    />
                </div>
            ))}
            <div className="text-center">
                <button className="bg-blue-500 text-white font-bold uppercase text-lg px-6 py-3 rounded shadow-lg hover:bg-blue-600 focus:outline-none focus:shadow-outline mt-4" type="submit">
                    Save Specials
                </button>
            </div>
        </form>
